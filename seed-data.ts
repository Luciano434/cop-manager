...
import { procedures, operationalSteps, copRequirements, procedureCopLinks } from "./drizzle/schema";
...
async function seed() {
  ...
  try {
    ...
    const copReqDescriptions: Record<string, string> = {
    ...
    const copRequirementIds: Record<string, number> = {};
    ...
    // Get the created procedure to get its ID
    const createdProc = await db.select().from(procedures).where(eq(procedures.code, "PR-01"));
    const procedureId = createdProc[0]?.id;
    console.log(`Created procedure PR-01 with ID: ${procedureId}`);

    if (!procedureId) {
      throw new Error("Failed to create procedure PR-01");
    }
